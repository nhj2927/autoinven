(function ($) {
  ...
  $(document).ready(function () {
      ...
      if (email_check.test(email) == false)
        ...
        $.ajax({
          ...
          success: function (rcvData) {
            if (rcvData.result == false) swalError('잠시후 다시 시도해주세요.');
            else {
              Swal.fire({
                icon: 'success',
                title: 'Success',
    ...
    $('#signup_button').click(function () {
      ...
      if (authFlag == false) swalError('이메일 인증을 완료해주세요.');
      else if (!email) swalError('이메일을 입력해주세요.');
      else if (!name) swalError('이름을 입력해주세요.');
      else if (name_check.test(name) == false)
        swalError('정확한 이름을 입력해주세요.');
      else if (!password) swalError('비밀번호를 입력해주세요.');
      else if (pw_check.test(password) == false)
        swalError(
          '최소8자리 영문 대소문자 및 숫자를 조합한 비밀번호를 입력해주세요.'
        );
      else if (password != pwcheck) swalError('비밀번호를 다시 확인해주세요.');
      else if (!phone) swalError('휴대폰 번호를 입력해주세요.');
